...
% Select single feature (index 58)
feature_idx = 58;
...
for i = 1:length(s_neg_train)
    ...
    [~, psi_L] = sgtfun_v3(Seq, k, labels, n);
    psi_L_mat_neg_train{i} = psi_L;
end
fprintf('Done with s_neg_train\n');

% Positive train set
for i = 1:length(s_pos_train)
    Seq = s_pos_train(i).Sequence;
    [~, psi_L] = sgtfun_v3(Seq, k, labels, n);
    psi_L_mat_pos_train{i} = psi_L;
end
fprintf('Done with s_pos_train\n');

% Negative val set
for i = 1:length(s_neg_val)
    Seq = s_neg_val(i).Sequence;
    [~, psi_L] = sgtfun_v3(Seq, k, labels, n);
    psi_L_mat_neg_val{i} = psi_L;
end
fprintf('Done with s_neg_val\n');

% Positive val set
for i = 1:length(s_pos_val)
    Seq = s_pos_val(i).Sequence;
    [~, psi_L] = sgtfun_v3(Seq, k, labels, n);
    psi_L_mat_pos_val{i} = psi_L;
end
fprintf('Done with s_pos_val\n');

% Negative test set
for i = 1:length(s_neg_test)
    Seq = s_neg_test(i).Sequence;
    [~, psi_L] = sgtfun_v3(Seq, k, labels, n);
    psi_L_mat_neg_test{i} = psi_L;
end
fprintf('Done with s_neg_test\n');

% Positive test set
for i = 1:length(s_pos_test)
    Seq = s_pos_test(i).Sequence;
    [~, psi_L] = sgtfun_v3(Seq, k, labels, n);
    psi_L_mat_pos_test{i} = psi_L;
end
fprintf('Done with s_pos_test\n');
...
metrics_table.CH_Index(:) = ch_index; % Same for all rows
...
for i = 1:length(s_neg_full)
    ...
    [~, psi_L] = sgtfun_v3(Seq, bestParams.k, best_labels, bestParams.n);
    psi_L_mat_neg_full{i} = psi_L;
end
...
function [labels, ch_index] = label_features(dataset, n)
    % Run k-means
    [idx, ~] = kmeans(dataset', n, 'Replicates', 5);
    labels = idx';
    
    % Compute Calinski-Harabasz Index
    eva = evalclusters(dataset', idx, 'CalinskiHarabasz');
    ch_index = eva.CriterionValues;
end
